...
async function findAll(req:Request,res:Response){
  try{
    const temporada = await em.find(Temporada, {})
...
async function findOne(req:Request,res:Response) { 
    try{
      ...
      const temporada = await em.findOneOrFail(Temporada, {id})
...
async function add(req:Request,res:Response){
  try{
    ...
    await em.flush()
    res.status(201).json({message: 'temporada created succesfully', data: temporada})
  }catch(error:any){
    res.status(500).json({ message: error.message})
  }
}
